...
var ImageAccessApp = App('ImageAccessApp', {
  ...
  downloadImageByDateId:function (date, imageId, filename, extension) {
    var self = this;
    var query = {date: date, _id: imageId};
    this.app.getImageDoc(query, function (err, imageDoc) {
      if (imageDoc) {
        imageDoc.type = 'application/octet-stream';
        filename = filename + '.' + extension;
        self.emit('getImageByDateId', null, imageDoc, {'content-disposition': 'attachment; filename="' + filename + '"'});
      } else {
        self.emit('getImageByDateId', err || {message: 'Image ' + imageId + ' not found.'});
      }
    });
  ...
  routes:{
    ...
    downloadImageByDateId:{method:'get', url:'/download/([0-9]{8})/id/([0-9a-zA-Z]{40})/(.*)\\.(jpg|png|gif)$'},
  ...
  routeResults:{
    getImageByDateId:function (err, imageDoc, headers) {
      if (err || !imageDoc) {
